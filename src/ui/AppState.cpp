...
void AppState::load() {
    ...
    m_setupCompleted = m_settings->value("setupCompleted", false).toBool();
...
void AppState::save() const {
    ...
    m_settings->setValue("setupCompleted", m_setupCompleted);
...
bool AppState::setSetupCompleted(bool completed) {
    if (m_setupCompleted == completed) return false;
